...
在使用GitHub的环境中，你可以配置Rancher以允许使用GitHub凭据登录。
...
5. 最后点击`Register application`
    ...
    > **什么是授权回调网址?** 使用外部身份验证时，实际上不会在你的应用程序中进行身份验证。相反，身份验证在外部进行(在本例中为GitHub)。在外部身份验证成功完成后，用户将通过`授权回调URL`重新进入应用程序。
...
2. 允许集群、项目以及授权用户和组成员
    ...
    配置为集群成员或项目成员的所有GitHub用户或组，以及本地用户都可以登录到Rancher。此外，你添加到`授权用户和组织`列表的任何GitHub用户或组都可以登录到Rancher。
