...
```bash
api_url='https://demo.cnrancher.com'
token='token-5zgl2:tcj5nvfq67rf55r722rl9x2lhr9mhwhgkrf6xbkqq9fx429xrwd4zx'
cluster_name='test'
...
curl -k -X POST \
  ...
  -d "$(post_data)" $api_url/v3/clusters
}

add_Node()
{
     cluster_ID=$( curl -s -k -H "Authorization: Bearer $token" \
         $api_url/v3/clusters | jq -r ".data[] | select(.name == \"$name\") | .id" )
     node_Command=$( curl -s -k -H "Authorization: Bearer $token" \
         $api_url/v3/clusters/${cluster_ID}/clusterregistrationtokens | jq -r .data[].nodeCommand )
}
```
