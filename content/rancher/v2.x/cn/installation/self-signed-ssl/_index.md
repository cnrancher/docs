...
1. 一键生成SSL证书脚本
    ...
    echo "1、生成CA证书"
    openssl req -newkey rsa:4096 -nodes -sha256 -keyout ca.key -x509 -days ${DATE} -out ca.crt \
    ...
    echo "生成服务证书"
    openssl x509 -req -days ${DATE} -in ${CN}.csr -CA ca.crt -CAkey ca.key -CAcreateserial -extfile extfile.cnf -out  ${CN}.crt
    ```
