...
Pod生命周期事件生成器是kubelet的一个函数，它为所有容器和Pod创建一个状态列表，然后在一个名为Relisting的过程中将其与容器和Pod的先前状态进行比较。这允许PLEG知道哪些PODs和容器需要同步。在1.2之前的版本中，这是通过轮询来完成的，并且是CPU密集型的。通过更改此方法，可以显著降低资源利用率，从而提高容器密度.
